...
function showModuleBreak(){
  ...
  // Calculate Module 1 score (first 27 questions)
  let module1Score = 0;
  for(let i = 0; i < 27 && i < questions.length; i++){
    const q = questions[i];
    // Check if answer is correct (selections stores index, correct_index is the correct option index)
    if(typeof selections[q.id] === 'number' && selections[q.id] === q.correct_index){
      module1Score++;
    }
  }
    ...
    <div style="text-align:center;padding:40px 20px">
      ...
      <div style="margin:24px 0">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px">
          ...
          <span style="font-size:14px;font-weight:bold;color:var(--accent)">${Math.round((module1Score/27)*100)}%</span>
        </div>
        <div style="background:#e0e7ff;height:8px;border-radius:4px;overflow:hidden">
          <div style="background:var(--accent);height:100%;width:${(module1Score/27)*100}%;transition:width 0.3s ease"></div>
        </div>
      </div>
      ...
      <div style="background:rgba(99,102,241,0.1);border-left:4px solid var(--accent);padding:16px;margin:24px 0;text-align:center;border-radius:4px">
        ...
        <p style="margin:0;font-size:28px;font-weight:bold;color:var(--accent)">${module1Score} / 27</p>
